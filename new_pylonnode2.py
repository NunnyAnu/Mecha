...
if __name__ == '__main__':
    action = PylonAction()
    a = action.checkcamera()
    if a:
        captured_image = action.capture()
        processed = action.preprocessimage(captured_image)
        plt.imshow(processed)
        plt.show()
        b = action.publishtodb(processed, 2)
